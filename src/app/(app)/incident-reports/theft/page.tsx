...
import { theftSchema, type TheftFormValues } from '@/lib/validations/incident-report';
import { createTheftReport } from '@/lib/api/incident-reports';
...
function YesNoRadio({
  ...
  return (
    <RadioGroup onValueChange={onChange} value={value} className="flex gap-6 pt-1">
...
export default function TheftReportPage() {
  ...
  const form = useForm<TheftFormValues>({
  ...
  const mutation = useMutation({
    mutationFn: (values: TheftFormValues) =>
      createTheftReport({
        ...values,
        involvedViolence: values.involvedViolence === 'true',
        propertyTaken: values.propertyTaken === 'true',
      }),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle>Roubo ou furto</CardTitle>
          ...
          <form onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>O bem chegou a ser subtraído?</FormLabel>
                    <FormControl>
                      <YesNoRadio value={field.value} onChange={field.onChange} />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="sm:col-span-2">
                    <FormLabel>Objetos roubados</FormLabel>
                    <FormControl>
                      <Textarea {...field} />
                    </FormControl>
                    <FormMessage />
